...
### Regex – hidden files and system files

```
/^\./
/^@eaDir/
```
Skips dot-files (`.DS_Store`, `.nomedia`) and Synology thumbnail folders (`@eaDir`).
...
### Regex – Synology system folders

```
/^@/
```
Skips all folders starting with `@` (e.g. `@eaDir`, `@Recycle`, `@tmp`).
...
| Input | Treated as |
|---|---|
| `IMG_001.jpg` | Exact match (case-sensitive) |
| `*.jpg` | Wildcard: any name ending in `.jpg` |
| `/regex/flags` | Regular expression with optional flags (`i` = case-insensitive) |
